...
export default function EditRestaurant() {
  ...
  const onSubmit = async (data: ICreateRestaurant) => {
    ...
    let payload = {
      ...data,
      established_date:
        typeof data.established_date === 'string'
          ? data.established_date
          : moment(data.established_date).format('YYYY-MM-DD'),
      restaurant_domains:
    ...
    const res = await RestaurantService.update(id || '', payload)
    setLoading(false)
    if (res.status) {
  ...
  const getSingleRestaurant = async () => {
    ...
    if (res.status) {
      let data = {
        ...res.data,
        restaurant_domains: res.data.restaurant_domains.map((domain) => domain.domain),
        restaurant_delivery_service: res.data.restaurant_delivery_service || {
          delivery_service_id: null,
        },
        plan: res.data.plan.id,
        established_date: moment(res.data.established_date),
        setting: {
          ...res.data.setting,
        },
      }
      if (data.restaurant_payment_service) {
        data.restaurant_payment_service = {
          ...data.restaurant_payment_service,
          payment_gateway: data?.restaurant_payment_service?.payment_service?.identifier || 0,
        }
      }
      methods.reset(data)
    }
        ...
        <FormProvider {...methods}>
          <Form onSubmit={methods.handleSubmit(onSubmit)}>
            <Row className={'h-100'}>
              ...
              <Col md={4}>
                <div className={'dfields crestaurant-fields restaurant-domain'}>
                  <Controller
