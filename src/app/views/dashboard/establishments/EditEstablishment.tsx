...
export default function EditEstablishment({
  ...
  const editId = idByProps || Number(id)
  ...
  const getSingleEstablishment = async () => {
    setLoader(true)
    const result = await EstablishmentServices.getById(editId)
    if (result.status) {
      const updatedTimeTable = Array(7)
        .fill({
          start_time: '',
          end_time: '',
        })
      ...
      const updateData: ICreateEstablishment = {
        ...
        establishment_order_types: orderTypes.map((orderType) => ({
          order_type: result.data.establishment_order_types.find(
            (singleOrderType) => singleOrderType.order_type === orderType.id
          )?.order_type,
        })),
        establishment_online_order_types: onlineOrderTypes.map((orderType) => ({
          online_order_type: result.data.establishment_online_order_types.find(
            (singleOnlineOrderType) => singleOnlineOrderType.online_order_type === orderType.id
          )?.online_order_type,
        })),
      }
  ...
  const onSubmit = async (data: ICreateEstablishment) => {
    ...
    if (latitude && longitude) {
      ...
      const res = await EstablishmentServices.updateEstablishment(editId, data)
      if (res.status) {
                                            ...
                                            render={({field: {name, value}}) => {
                                              return (
                                                <DashCheckboxWithValue
                                                  checkedInput={
                                                    !!updatedOrderTypes[index]?.order_type
                                                  }
                                        ...
                                        <div className={'dfields discount-fields'}>
                                          <Controller
                                            ...
                                            render={({field: {name, value}}) => (
                                              <DashCheckboxWithValue
                                                checkedInput={
                                                  !!updatedOnlinedOrderTypes[index]
                                                    ?.online_order_type
                                                }
                                                setValue={handleChangeCheckBox}
                                                name={name}
                  ...
                  <Col className={'mb-4'} md={12}>
                    ...
                    >
                      ...
                      <ThemeButton
                        ...
                        text={isInitialSetup ? 'Save & Next' : 'Update'}
                      />
                    </div>
